...
app = FastAPI(title="AURUM v2 API", version="0.5.0")
...
def update_memory_from_chat(history: List[Dict[str, str]]) -> dict:
    if len(history) < 8:
        return load_memory()
    ...
    prompt = "\n".join(
        [
            ...
            "기존 기억과 최근 대화를 보고 앞으로도 유용한 프로젝트 맥락만 압축해서 갱신한다.",
            ...
            "",
            "기존 중요 사실:",
            json.dumps(mem.get("facts", []), ensure_ascii=False),
            "",
            "최근 대화:",
            recent,
            "",
            "반드시 JSON만 출력한다.",
            '형식: {"summary":"장기적으로 유용한 현재 프로젝트 요약","facts":["중요 사실 1","중요 사실 2"]}',
        ]
    )
...
def get_weather() -> str:
    try:
        response = requests.get("https://wttr.in/Seoul?format=3", timeout=20)
        response.raise_for_status()
...
def choose_model(message: str, selected_model: str) -> tuple[str, str]:
    ...
    if selected_model and selected_model != "AUTO" and selected_model in allowed:
...
def run_python_detail(path_text: str) -> dict:
    ...
    if not target.exists():
        return {
            "ok": False,
            "output": f"실행 실패: 파일이 없습니다: {target}",
            "returncode": -1,
        }
    ...
    if target.suffix.lower() != ".py":
        return {
            "ok": False,
            "output": "실행 실패: 현재는 .py 파일만 실행 지원합니다.",
            "returncode": -1,
        }
...
def make_prompt(message: str, history: List[Dict[str, str]], model: str, reason: str) -> str:
    return "\n".join(
        [
            ...
            "- 코드블록 안에는 설명, bash 명령어, 마크다운 문장, heredoc 종료문자를 넣지 않는다.",
...
def auto_fix_code(original_message: str, code: str, run_output: str, code_path: str, max_rounds: int = 2) -> tuple[str, str, str]:
    ...
    for round_no in range(1, max_rounds + 1):
        ...
        fixed_answer = call_ollama(MODEL_WORK, fix_prompt, timeout=600)
        fixed_code = extract_code_block(fixed_answer)

        if not fixed_code:
            logs.append(f"[자동 수정 {round_no}] 수정 코드블록 감지 실패")
            break

        current_code = fixed_code
        save_result = save_code(current_code, code_path)
        run_detail = run_python_detail(code_path)
        ...
        if run_detail["ok"]:
            return current_code, "\n\n".join(logs), run_detail["output"]

        run_output = run_detail["output"]

    final_run = run_python_detail(code_path)
    return current_code, "\n\n".join(logs), final_run["output"]


def run_chat_logic(req: ChatRequest, auto_fix: bool = False) -> ChatResponse:
    ...
    model, reason = choose_model(message, req.selected_model)
    ...
    if code:
        ...
        first_run = run_python_detail(code_path)
        run_result = first_run["output"]

        answer += "\n\n[AURUM 자동 처리]\n"
        answer += "1. 코드 감지 완료\n"
        answer += f"2. {save_result}\n"
        answer += f"3. 1차 실행 결과:\n{run_result}"

        if auto_fix and not first_run["ok"]:
            fixed_code, fix_log, final_run = auto_fix_code(
    ...
    return ChatResponse(
        answer=answer,
        model=model,
        reason=reason + (" + 자동 수정" if auto_fix else ""),
        history=history,
        last_code=last_code,
        code_path=code_path,
        run_result=run_result,
...
def chat(req: ChatRequest):
    return run_chat_logic(req, auto_fix=False)


@app.post("/chat-auto", response_model=ChatResponse)
def chat_auto(req: ChatRequest):
    return run_chat_logic(req, auto_fix=True)
...
def tool_pdf_ocr(file: UploadFile = File(...), pages: str = Form("1")):
    ...
    safe_pages = (
        page_text.replace(",", "_").replace("-", "to").replace(" ", "")
        if page_text
        else "all"
    )
...
def tool_cad_kml(file: UploadFile = File(...)):
    ...
    if ext == ".kml":
        ...
        return {
            "status": "error",
            "message": "KML 또는 DXF 파일만 지원합니다.",
        }

    log = run_cmd(
        [
            "python",
            str(SCRIPTS / "cad_kml_convert.py"),
            mode,
            str(path),
            str(out),
        ]
